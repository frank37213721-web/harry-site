...
summary: >-
  ...
  | **AI 蜜月期結束**                       | 模型越來越強，但個人的用法與企業的 ROI 都沒有跟著提升           |

  | **FDE（Forward Deployed Engineer）** | 工程師駐點到客戶現場，從真實痛點反推出可泛用的產品               |

  | **Harness Engineering**            | 給 AI 一副「馬具」：規劃 → 評估 → 迭代優化，三條韁繩         |

  | **訓練組／測試組**                        | 把素材對半分，用真實成果反覆打臉 AI 並要求它優化，準確率停在 80–85% |

  | **Overfitting**                    | 準確率太高反而變成逐字模仿，產出會很「乾」                   |

  | **指揮的三個層次**                        | 提示詞工程 → 上下文工程 → 指揮（何時用什麼、用哪個模型）         |

  | **不要從 0 開始**                       | 先讓 AI 找到該領域的 best practice，再從那裡開始討論     |

  | **肉與骨頭**                           | 案例是肉、理論架構是骨；只有肉，AI 只會得到一堆雜訊             |

  | **思考外部化**                          | 用思考習慣拆解一個人的決策，讓它可被驗證、可被優化               |

  | **第一性原理**                          | ＝問對問題＋拆解問題；分清楚哪些是障礙、哪些是限制               |

  | **Loop Engineering**               | 大專案切成許多小迴圈，每個迴圈都有審核者才往下走                |

  | **Echo team／Delta team**           | 建設性批判＋抽象化能力／快速做原型、快速試錯                  |
draft: false
